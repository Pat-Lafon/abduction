type t = {
  ...
  (* which candidate names to select from that file *)
  (* TODO: maybe unnneed give that we no longer use templates for recursive arguments?*)
  name_to_avoid : string list; [@default [ "inv"; "mx"; "lo"; "hi" ]]
  rlimit : int option; [@default None]
}
